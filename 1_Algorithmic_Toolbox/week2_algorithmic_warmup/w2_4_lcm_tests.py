...
sample_input_1=[6,8]
sample_output_1 = 24
sample_input_2=[761457,614573]
sample_output_2 = 467970912861
boundary_input=[1,int(1e7)]
stress_tests_boundary = [1, 1000]

print(f"\n{Style.BRIGHT}Least Common Multiple Algorithm.{Style.RESET_ALL}\n")
table_data = [['Task', 'Given two integers a and b, find their least common multiple.'],
                ['Constraints', f'{boundary_input[0]}<=a,b<={boundary_input[1]}'],
                ['Output Format', 'Output the last least common multiple of a and b'],
...
while True:
    ...
    if choice == '1':
        ...
        good_model_test(_input=[[boundary_input[0], random.randint(boundary_input[0], boundary_input[1])],
                                 [random.randint(boundary_input[0], boundary_input[1]), boundary_input[0]], 
                                 [boundary_input[0],boundary_input[1]]], test_name="Boundary")
    elif choice == '3':
        stress_tests(number_of_tests=number_of_tests, low=stress_tests_boundary[0], high=stress_tests_boundary[1])
    elif choice == '4':
        PROMPT_USER = True
        good_model_test(_input=[sample_input_1], test_name="Sample", known_result=sample_output_1)
        good_model_test(_input=[sample_input_2], test_name="Sample", known_result=sample_output_2)
