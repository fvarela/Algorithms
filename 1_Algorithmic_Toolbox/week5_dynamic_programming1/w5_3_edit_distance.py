...
def model_good(first, second, debug=False):
    ...
    matrix = []
    matrix.append([i for i in range(len(first)+1)])
    ...
    for i in range(0, len(second)):
        for j in range(0, len(first)):
            if debug: print(f"Comparing i:{i} and j:{j}")
            insertion = matrix[i+1][j]+1
            deletion = matrix[i][j+1]+1
            match = matrix[i][j]
            mismatch = matrix[i][j]+1
            if first[j] == second[i]:
                matrix[i+1][j+1]=min(insertion, deletion, match)
            else:
                matrix[i+1][j+1] = min(insertion, deletion, mismatch)
...
if __name__ == '__main__':
    ...
    sample_input_1= ['ab','ab']
    sample_output_1 = 0
    sample_1_text = ''
    sample_input_2=['short','ports']
    sample_output_2 =3 
    sample_2_text = ''
    sample_input_3=['distance', 'editing']
    sample_output_3 =5 
    sample_3_text = ''
    ...
    print(f"\n{Style.BRIGHT}Edit Distance Algorithm.{Style.RESET_ALL}\n")
    choices = []
